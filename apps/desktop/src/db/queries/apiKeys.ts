...
export function listApiKeys(db: Database.Database): ApiKeyEntry[] {
  const rows = db
    .prepare("SELECT id, provider, alias, created_at FROM api_keys ORDER BY created_at")
...
export function upsertApiKey(db: Database.Database, key: ApiKeyEntry): void {
  db.prepare(
    `INSERT INTO api_keys (id, provider, alias, created_at) VALUES (?, ?, ?, ?)
     ON CONFLICT(id) DO UPDATE SET provider = excluded.provider, alias = excluded.alias`
  ).run(key.id, key.provider, key.alias ?? null, key.createdAt);
}
