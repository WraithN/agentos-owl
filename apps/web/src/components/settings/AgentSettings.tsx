...
import { Plus, X, ChevronLeft, ChevronRight, Pencil, Trash2, Database, ChevronDown, Loader2, Brain, AlertTriangle } from 'lucide-react';
import { cn } from '@/lib/utils';
import { AVAILABLE_KBS } from '@/data/mockData';
import type { Agent } from '@/types';
import { useT } from '@/lib/i18n';
import { listAgents, saveAgent, deleteAgent } from '@/services/electron';
import { useChatLlmModels } from '@/hooks/use-llm-default';
import { emitAgentsChanged } from '@/hooks/use-agents';
...
const COLOR_PALETTE = ['#00F5A0', '#00D4FF', '#A78BFA', '#F472B6', '#FB923C', '#34D399'];
        ...
        <div className="flex flex-wrap gap-1.5">
          {value.map(kb => (
            <span key={kb.id} className="flex items-center gap-1 text-[11px] bg-cyan-500/12 text-cyan-600 dark:text-cyan-300 px-2 py-0.5 rounded-full border border-cyan-500/25">
              ...
              <button type="button" onClick={() => onChange(value.filter(k => k.id !== kb.id))} className="hover:text-rose-400 transition-colors ml-0.5"><X className="w-2.5 h-2.5" /></button>
            </span>
          ))}
        </div>
      )}
        ...
        {open && (
          <div className="absolute z-20 top-full mt-1 w-full rounded-xl border border-[var(--border-subtle)] overflow-hidden shadow-xl"
            ...
            {AVAILABLE_KBS.map(kb => {
              ...
              return (
                <button type="button" key={kb.id} onClick={() => toggle(kb)}
...
function ModelSelect({ value, onChange, errored, className }: ModelSelectProps) {
  const { ready, models, defaultModel } = useChatLlmModels();
  const hasOptions = models.length > 0;

  // 如果当前 value 不在选项内（例如来自旧数据），保留显示一个降级 option
  const valueInOptions = !value || models.some(m => m.name === value);
  ...
  return (
    <div className="space-y-1">
      <select
        ...
        disabled={!hasOptions}
        className={cn(inputCls(errored ? 'err' : undefined), 'appearance-none cursor-pointer', !hasOptions && 'opacity-60 cursor-not-allowed', className)}
      >
        {!value && <option value="">— 请选择模型 —</option>}
        {!valueInOptions && value && (
          <option value={value}>{value}（已失效）</option>
        )}
        {models.map(m => (
      ...
      {ready && !hasOptions && (
        <p className="text-[10px] text-amber-500 dark:text-amber-400 flex items-center gap-1">
          ...
          尚未在「LLM 配置」中添加任何对话模型
        </p>
      )}
      {ready && hasOptions && defaultModel && !value && (
        <p className="text-[10px] text-slate-500 flex items-center gap-1">
          <Brain className="w-3 h-3" />
          默认对话模型：{defaultModel.name}
        </p>
      )}
    </div>
  );
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <motion.div initial={{ opacity: 0, scale: 0.96, y: 16 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.96, y: 16 }} transition={{ duration: 0.18 }}
        ...
        <div className="flex items-center justify-between px-5 py-4" style={{ borderBottom: `1px solid ${DIALOG_BD}` }}>
          ...
          <button type="button" onClick={onClose} className="p-1 rounded-lg text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 hover:bg-black/8 dark:hover:bg-white/8 transition-colors"><X className="w-4 h-4" /></button>
        </div>
        <div className="p-5 space-y-4 max-h-[70vh] overflow-y-auto">
          ...
          <div>
            ...
            <ModelSelect
              value={form.model}
              onChange={v => { setForm(f => ({ ...f, model: v })); setErrors(p => ({ ...p, model: undefined })); }}
              errored={!!errors.model}
            />
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5 mb-2">
              {form.tools.map(t => (
                <span key={t} className="flex items-center gap-1 text-[10px] bg-cyan-500/12 text-cyan-600 dark:text-cyan-300 px-2 py-0.5 rounded-full border border-cyan-500/25">
                  {t}<button type="button" onClick={() => setForm(f => ({ ...f, tools: f.tools.filter(x => x !== t) }))} className="hover:text-rose-400 transition-colors"><X className="w-2.5 h-2.5" /></button>
                </span>
              ))}
            </div>
            <div className="flex gap-2">
              ...
              <button type="button" onClick={() => addTool(toolInput)} disabled={!toolInput}
        ...
        <div className="flex items-center justify-end gap-2 px-5 py-4" style={{ borderTop: `1px solid ${DIALOG_BD}` }}>
          <button type="button" onClick={onClose} className="px-4 py-2 text-xs text-slate-500 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-black/8 dark:hover:bg-white/8 rounded-xl transition-colors">取消</button>
          <button type="button" onClick={handleSubmit} className="flex items-center gap-1.5 px-4 py-2 text-xs font-semibold text-white btn-aurora rounded-xl">
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.96 }} transition={{ duration: 0.15 }}
        ...
        <div className="flex justify-end gap-2">
          <button type="button" onClick={onCancel} className="px-4 py-2 text-xs text-slate-500 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-black/8 dark:hover:bg-white/8 rounded-xl transition-colors">取消</button>
          <button type="button" onClick={onConfirm} className="px-4 py-2 text-xs font-semibold text-white bg-rose-500 hover:bg-rose-600 rounded-xl transition-colors">删除</button>
        </div>
      </motion.div>
    </div>
  );
...
export default function AgentSettings() {
  ...
  const { defaultModel } = useChatLlmModels();
  const t = useT();
  ...
  useEffect(() => {
    ...
    listAgents()
      ...
      .catch((err) => {
        ...
        if (!cancelled) toast.error('加载智能体失败');
      })
      .finally(() => {
  ...
  const pageAgents = useMemo(
    () => agents.slice(safePage * PAGE_SIZE, (safePage + 1) * PAGE_SIZE),
    [agents, safePage]
  );

  const emptyForm: AgentFormData = useMemo(
    () => ({
      name: '',
      description: '',
      enabled: true,
  ...
  async function handleCreate(data: AgentFormData) {
    const newAgent: Agent = {
      ...
      color: COLOR_PALETTE[agents.length % COLOR_PALETTE.length],
  ...
  async function handleToggleEnabled(agent: Agent) {
    ...
    // 乐观更新
    setAgents((prev) => prev.map((a) => (a.id === agent.id ? next : a)));
    try {
  ...
  async function handleInlineModelChange(agent: Agent, model: string) {
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <button type="button" onClick={() => setDialogOpen(true)} className="flex items-center gap-1.5 px-4 py-2 text-xs font-semibold text-white btn-aurora rounded-xl">
      ...
      {agents.length === 0 ? (
        <div className="glass rounded-2xl p-12 text-center">
          <p className="text-sm text-slate-500">还没有任何智能体，点击「新建 Agent」开始配置。</p>
        </div>
      ) : (
          ...
          {pageAgents.map((agent, i) => (
            <motion.div key={agent.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }}
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex items-center gap-0.5 shrink-0" onClick={e => e.stopPropagation()}>
                  <button type="button" onClick={() => handleToggleEnabled(agent)}
                    ...
                    className={cn('w-9 h-5 rounded-full relative transition-all duration-200 shrink-0 mr-0.5', agent.enabled ? 'bg-gradient-to-r from-cyan-500 to-purple-600' : 'bg-black/15 dark:bg-white/15')}>
                    <div className={cn('absolute top-0.5 w-4 h-4 rounded-full bg-white transition-all duration-200', agent.enabled ? 'left-4' : 'left-0.5')} />
                  </button>
                  <button type="button" onClick={() => setEditTarget(agent)}
              ...
              <div onClick={e => e.stopPropagation()}>
                ...
                <ModelSelect
                  value={agent.model}
                  onChange={(m) => handleInlineModelChange(agent, m)}
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1 mt-1">
                  {agent.tools.map(tool => (
                    <span key={tool} className="text-[10px] bg-black/5 dark:bg-white/8 text-slate-600 dark:text-slate-300 px-2 py-0.5 rounded-full border border-[var(--border-subtle)]">{tool}</span>
                  ))}
                  <button type="button" onClick={e => { e.stopPropagation(); setEditTarget(agent); }}
              ...
              <div onClick={e => e.stopPropagation()}>
                ...
                <p className="mt-1 bg-black/5 dark:bg-white/5 border border-[var(--border-subtle)] rounded-lg px-3 py-2 text-xs text-slate-700 dark:text-slate-300 leading-relaxed min-h-[2.5rem] whitespace-pre-wrap">
                  {agent.triggerRule || <span className="text-slate-500">未配置触发规则</span>}
                </p>
              </div>
            </motion.div>
          ))}
      ...
      <AnimatePresence>
        {editTarget && (
          <AgentDialog title={`编辑 Agent · ${editTarget.name}`} confirmLabel="保存修改"
            initial={{ name: editTarget.name, description: editTarget.description, enabled: editTarget.enabled, prompt: '', model: editTarget.model, tools: [...editTarget.tools], triggerRule: editTarget.triggerRule, knowledgeBases: [] }}
