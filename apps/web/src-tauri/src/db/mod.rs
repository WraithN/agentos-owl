...
pub fn db_path(app: &AppHandle) -> Result<PathBuf, ActaError> {
    let mut dir = app
        .path()
        .app_data_dir()
        .map_err(|e| ActaError::Config(format!("无法获取应用数据目录: {}", e)))?;
    std::fs::create_dir_all(&dir)
        .map_err(|e| ActaError::Config(format!("无法创建应用数据目录: {}", e)))?;
    dir.push("owl_os.db");
    Ok(dir)
}

pub fn init_db(app: &AppHandle) -> Result<Connection, ActaError> {
    let path = db_path(app)?;
    let conn = Connection::open(&path)
        .map_err(|e| ActaError::Db(format!("无法打开数据库 {}: {}", path.display(), e)))?;
    run_migrations(&conn)?;
    Ok(conn)
}

fn run_migrations(conn: &Connection) -> Result<(), ActaError> {
    conn.execute_batch(
        r#"
    ...
    .map_err(|e| ActaError::Db(format!("迁移失败: {}", e)))?;
    Ok(())
}
