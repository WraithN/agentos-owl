...
export interface SliceRule {
  id: string; name: string; strategy: Strategy;
  chunkSize: number; overlap: number; separator: string;
  preprocess: string[]; description: string;
}

/* ── Mock 规则 ────────────────────────────────────────── */
export const DEFAULT_SLICE_RULES: SliceRule[] = [
  { id: 'sr-1', name: '固定长度切片', strategy: 'fixed',     chunkSize: 512, overlap: 64,  separator: '\n',   preprocess: ['clean', 'dedup'], description: '按固定 token 数切割，适合格式均匀的文档' },
  { id: 'sr-2', name: '段落切片',     strategy: 'paragraph', chunkSize: 800, overlap: 80,  separator: '\n\n', preprocess: ['clean', 'table'], description: '按段落边界切割，保留语义完整性' },
  { id: 'sr-3', name: '语义切片',     strategy: 'semantic',  chunkSize: 600, overlap: 100, separator: '',     preprocess: ['clean', 'filter', 'dedup'], description: '基于语义相似度自动分割，效果最佳' },
];

export const STRATEGY_LABELS: Record<Strategy, string> = {
  fixed: '固定长度', sentence: '按句子', paragraph: '按段落', semantic: '语义分割',
};
...
/* ── 上传弹窗（主） ──────────────────────────────────── */
export default function KBUploadDialog({ kbName, rules, onClose }: {
