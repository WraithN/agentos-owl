...
const TOOL_TYPES: ToolTypeOption[] = [
  {
    ...
    fields: [
      ...
      { key: 'category',  label: '分类',       type: 'select',   options: ['搜索', '代码', '数据分析', '文档', '通信', '通用'], required: true, allowCreate: true },
      { key: 'endpoint',  label: '服务地址',   type: 'text',     placeholder: 'http://localhost:8080/mcp', required: true, hint: 'SSE 或 HTTP Streamable 均可' },
  ...
  },
  {
    ...
    fields: [
      ...
      { key: 'category',    label: '分类',       type: 'select',   options: ['搜索', '代码', '数据分析', '文档', '通信', '通用'], required: true, allowCreate: true },
      { key: 'command',     label: '执行命令',   type: 'code',     placeholder: 'python /workspace/scripts/analyze.py --input {{input}}', required: true, hint: '使用 {{参数名}} 插入动态参数' },
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-0.5">
                              ...
                              <span className={cn('text-[10px] px-1.5 py-0.5 rounded border font-mono', type.badgeColor)}>{type.badge}</span>
                            </div>
              ...
              <div className="flex items-center justify-end gap-3 px-5 py-4 border-t border-[var(--border-subtle)] shrink-0">
                ...
                <button
                  ...
                  className={cn(
                    ...
                    isFormValid && !saving
                      ? 'btn-aurora'
                      : 'bg-black/5 dark:bg-white/8 text-slate-400 cursor-not-allowed'
                  )}
                >
                  {saving ? (
        ...
        adding ? (
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              />
              <button onClick={confirm} className="px-3 py-2 text-xs font-medium text-white bg-cyan-500 rounded-xl hover:bg-cyan-600 transition-colors">确认</button>
              <button onClick={() => setAdding(false)} className="px-3 py-2 text-xs text-slate-500 hover:text-slate-700 rounded-xl transition-colors">取消</button>
            </div>
